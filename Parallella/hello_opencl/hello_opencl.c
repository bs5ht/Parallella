...
{ 
   ...
   //populates the number of platforms
   clGetPlatformIDs( 0,0,&nplatforms);
   //allocates memory for the platforms
   platforms = (cl_platform_id*)malloc(nplatforms*sizeof(cl_platform_id));
   //fills memory space(platforms) with the id's of platforms
   clGetPlatformIDs( nplatforms, platforms, 0);
   //the for loop iterates one time, printing out "coprthr"
